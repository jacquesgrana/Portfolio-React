...
const FilterTagComponent = (props: FilterTagComponentProps) => {
    return(
        ...
        <Stack className="mt-0 mb-0 d-flex flex-wrap justify-content-center flex-grow-1" direction="horizontal" gap={2}>
            {props.tagsToDisplay.map((tag: ITag) => (
                <span key={tag.id} onClick={() => props.clickOnTag(tag)} className="pointer-hover"
                title={`cliquer pour enlever ${tag.name}.`}>
                    <Tag 
                        key={tag.id} 
                        tag={tag} 
                    />
        ...
        <div className="d-flex justify-content-center gap-4 mt-0 mb-0">
